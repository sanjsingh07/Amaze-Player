...
package com.sanj.amaze.dataloaders;
...
public class PlaylistLoader {
    ...
    public static List<Playlist> getPlaylists(Context context, boolean defaultIncluded) {
        ...
        if (mCursor != null && mCursor.moveToFirst()) {
            do {
                ...
                final int songCount = amazeUtils.getSongCountForPlaylist(context, id);
    ...
    private static void makeDefaultPlaylists(Context context) {
        ...
        /* Last added list */
        final Playlist lastAdded = new Playlist(amazeUtils.PlaylistType.LastAdded.mId,
                resources.getString(amazeUtils.PlaylistType.LastAdded.mTitleId), -1);
        mPlaylistList.add(lastAdded);

        /* Recently Played */
        final Playlist recentlyPlayed = new Playlist(amazeUtils.PlaylistType.RecentlyPlayed.mId,
                resources.getString(amazeUtils.PlaylistType.RecentlyPlayed.mTitleId), -1);
        mPlaylistList.add(recentlyPlayed);

        /* Top Tracks */
        final Playlist topTracks = new Playlist(amazeUtils.PlaylistType.TopTracks.mId,
                resources.getString(amazeUtils.PlaylistType.TopTracks.mTitleId), -1);
